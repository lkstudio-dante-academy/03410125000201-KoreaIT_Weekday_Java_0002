 ...
 * 해당 공식에 의해 오차가 발생한다는 것을 알 수 있다. (+ 즉, 부동 소수점 방식은 비트를 바로 수로 변환하는 것이
 ...
 *
 * 변수 (Variable) 란?
 * - 데이터를 저장하거나 읽어들일 수 있는 공간을 의미한다. (+ 즉, 변수를 활용하면 특정 데이터를 저장 후
 ...
 * 상수 선언 이후에 데이터를 할당하면 컴파일 에러가 발생한다는 것을 의미한다.)
 *
 * Java 이름 작성 규칙
...
public class CE01Example_02 {
	...
	public static void start(String[] args) {
		 ...
		 *
		 * 형식화 된 문장을 출력하기 위해서는 서식 문자라는 개념이 필요하며 해당 문자는 자료형에 따라
		 * 다양한 종류가 존재한다.
		 *
		 * Java 서식 문자 종류
		 * - %c : char
		 ...
		 *
		 * 위와 같이 서식 문자는 문장이 과정에서 데이터로 치환되서 출력된다는 것을 알 수 있다. (+ 즉,
